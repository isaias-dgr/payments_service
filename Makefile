SHELL := /bin/bash
project:=ecommerce_service
project-path:=github.com/isaias-dgr/ecommerce_service
service:=payments
ENV?=dev
...
migration-create: start
	docker compose -p ${project} exec ${service} goose -dir ./migrations postgres "${POSTGRES_CONN}" create $(name) sql
	sudo chown -R $$USER ./migrations

.PHONY: migrate
migrate: start
	docker compose -p ${project} exec ${service} goose -dir ./migrations postgres "${POSTGRES_CONN}" up

.PHONY: rollback
rollback: start
	docker compose -p ${project} exec ${service} goose -dir ./migrations postgres "${POSTGRES_CONN}" down

.PHONY: reset-db
reset-db: start
	docker compose -p ${project} exec ${service} goose -dir ./migrations postgres "${POSTGRES_CONN}" reset
...
.PHONY: postgres
postgres:
	docker compose -p ${project} exec ${service}-db psql -U ${POSTGRES_USER} -W  
